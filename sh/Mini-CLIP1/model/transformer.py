...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, fearure_layers=None, visual_prompt=None):
        ...
        # layer 개수만큼 Transformer Block 반복
        for i in range(len(self.resblocks)):
            ...
            # 추출할 feature_layers를 설정한 경우 → 해당 layer의 feature를 out에 저장
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, feature_layers=[24], visual_prompt=None):
        ...
        # 4. Transformer 입력전 정규화 및 차원 변환
        ## [B, N+1, D] → [N+1, B, D]
        x = self.ln_pre(x)
        x = x.permute(1, 0, 2)

        # 5. Transformer Block에 입력
