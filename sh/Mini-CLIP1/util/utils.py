            ...
            if torch.sum(labels) >= 1:
                ...
                for p in pack:
                    if p[-2] != 0:
                        ...
                        result_path = os.path.join(arg.result_dir, 'vis', '{}-shot'.format(args.fewshot), test_dataset.dataset_name, test_dataset.category)
...
def eval_all_class(clip_model: CLIP, dataset_name, test_dataset, args, logger, device):
    ...
    for category in test_dataset.categories:
        ...
        if args.vis != 0:
            ...
            logger.info("{}\t{}".format(category, res_values_str))
