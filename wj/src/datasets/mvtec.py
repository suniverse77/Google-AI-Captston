...
def load_mvtec(category, k_shot, experiment_indx, *, use_full_train_if_k_shot_zero=False):
    ...
    if use_full_train_if_k_shot_zero and k_shot == 0:
...
class MvtecDataset(BaseDataset):
    ...
    def __init__(
        ...
        *,
        ...
        test_defect_types: Optional[List[str]] = None,
    ) -> None:
        ...
        self.test_defect_types = test_defect_types

        if root is not None:
            self.set_root(root)

        (train_data, test_data) = load_mvtec(
            category,
            k_shot,
            experiment_idx,
        ...
        # 특정 파일 번호 및 defect_type으로 필터링 (test split에만 적용)
        if split == "test":
            filtered_indices = []
            for idx, (img_path, defect_type) in enumerate(zip(self.image_paths, self.defect_types)):
                # defect_type 필터링
                if self.test_defect_types is not None and len(self.test_defect_types) > 0:
                    if defect_type not in self.test_defect_types:
                        continue
                
                # 파일 번호 필터링
                if self.test_file_numbers is not None and len(self.test_file_numbers) > 0:
                    ...
                    # 파일 번호 필터링이 없으면 defect_type만 확인
                    filtered_indices.append(idx)
