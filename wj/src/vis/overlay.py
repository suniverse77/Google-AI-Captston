...
def plot_overlay_grid(
    *,
    ...
    # 파일명이 있으면 전체 figure 제목에 표시
    if filename:
        fig.suptitle(f"File: {filename}", fontsize=12, y=0.98)
...
def save_overlay_examples(
    ...
    *,
    ...
    for idx, sample in enumerate(samples):
        ...
        # 실제 파일 번호가 있으면 사용, 없으면 enumerate 인덱스 사용
        file_number = sample.get("file_number")
        if file_number is not None:
            overlay_idx = file_number
        else:
            overlay_idx = idx
        save_path = output_dir / f"overlay_{overlay_idx:03d}.png"
